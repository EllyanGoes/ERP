...
function AprovacaoCard({
  ...
  const prio      = sc ? (PRIORIDADE[sc.prioridade] ?? { label: String(sc.prioridade), cls: "text-muted-foreground" }) : null;
  ...
  if (!sc && cot) {
    ...
    return (
      <div
        className={cn(
          "bg-card rounded-xl border shadow-sm overflow-hidden transition-all p-4",
          effectiveStatus === "APROVADO" ? "border-success/30 bg-success/10" :
          effectiveStatus === "REPROVADO" ? "border-danger/30 bg-danger/10" :
          "border-border hover:border-border"
        )}
      >
        <div className="flex items-center gap-2 flex-wrap">
          <Link href={`/suprimentos/cotacoes/${cot.id}`} className="font-semibold text-foreground hover:text-info transition-colors text-sm">
            {cot.nome || cot.numero}
          </Link>
          <span className="text-xs px-2 py-0.5 rounded-full bg-warning/10 text-warning font-medium border border-amber-100">Cotação · Pedido de Compras</span>
          {cot.necessidade && <span className="text-xs text-muted-foreground">SC {cot.necessidade.numero}</span>}
        </div>
        <div className="mt-2 text-sm text-muted-foreground">
          {venceNome ? (
            <>Fornecedor proposto: <span className="font-medium text-foreground">{venceNome}</span>
        ...
        {showObs && isPendente && (
          <textarea
            ...
            className="mt-3 w-full h-16 rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        )}
        ...
        {isPendente ? (
          <div className="mt-3 flex items-center gap-2">
            ...
            <button onClick={() => responder("REPROVAR")} disabled={!!loading}
              className="px-3 py-1.5 rounded-lg border border-danger/30 text-danger text-sm font-medium hover:bg-danger/10 disabled:opacity-60">
  ...
  return (
    <div
      className={cn(
        ...
        done === "APROVADO"  || item.status === "APROVADO"  ? "border-success/30 bg-success/10" :
        done === "REPROVADO" || item.status === "REPROVADO" ? "border-danger/30 bg-danger/10" :
        "border-border hover:border-border"
      )}
    >
      {/* Header */}
      <div className="flex items-start gap-3 px-4 pt-4 pb-3">
        ...
        {isPendente && (
          <button
            onClick={onToggle}
            className="mt-0.5 shrink-0 text-muted-foreground/60 hover:text-blue-500 transition-colors"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <Link
              ...
              className="font-semibold text-foreground hover:text-info transition-colors text-sm"
            >
              {sc.numero}
            </Link>
            {item.etapaNome && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-info/10 text-info font-medium border border-info/20">
                {item.etapaNome}
              </span>
            )}
        ...
        <div className="shrink-0">
          {effectiveStatus === "APROVADO"  && <span className="inline-flex items-center gap-1 text-xs font-medium text-success bg-success/15 px-2.5 py-1 rounded-full"><CheckCircle2 className="w-3 h-3" /> Aprovado</span>}
          {effectiveStatus === "REPROVADO" && <span className="inline-flex items-center gap-1 text-xs font-medium text-danger bg-danger/15 px-2.5 py-1 rounded-full"><XCircle className="w-3 h-3" /> Reprovado</span>}
          {isPendente                      && <span className="inline-flex items-center gap-1 text-xs font-medium text-warning bg-warning/15 px-2.5 py-1 rounded-full"><Clock className="w-3 h-3" /> Pendente</span>}
        </div>
      </div>
      ...
      {sc.motivo && (
        <div className="px-4 pb-2">
          <p className="text-sm text-foreground">
            <span className="font-medium text-muted-foreground">Motivo:</span> {sc.motivo}
          </p>
        </div>
      )}
      ...
      {sc.justificativa && (
        <div className="px-4 pb-2">
          <p className="text-sm text-muted-foreground">
            <span className="font-medium text-muted-foreground">Descrição:</span> {sc.justificativa}
          </p>
        </div>
      )}
      ...
      {sc.itens.length > 0 && (
        <div className="px-4 pb-3 flex flex-wrap gap-1.5">
          {sc.itens.map((it, i) => {
            ...
            return (
              <span key={i} className="text-xs bg-muted border border-border rounded-lg px-2.5 py-1 text-foreground">
                {it.item.descricao}
                <span className="text-muted-foreground ml-1">{qtd.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 3 })} {un}</span>
              </span>
            );
          })}
        </div>
      ...
      {item.status === "REPROVADO" && item.observacao && (
        <div className="mx-4 mb-3 p-3 bg-danger/10 border border-danger/20 rounded-lg text-xs text-danger">
      ...
      {isPendente && !bulkActive && (
        <div className="border-t border-border px-4 py-3 bg-muted/60">
          {error && (
          ...
          {showObs && (
            <div className="mb-3">
              <Textarea
                ...
                rows={2}
                className="text-sm resize-none border-danger/30 focus:border-red-400"
              />
            </div>
          )}
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" disabled={!!loading}
              ...
              className="border-danger/30 text-danger hover:bg-danger/10 hover:border-red-300 gap-1.5">
            ...
            {showObs && (
              <Button size="sm" variant="ghost" onClick={() => { setShowObs(false); setObs(""); }} className="text-muted-foreground">
                Cancelar
              </Button>
            )}
            <Link href={`/compras/necessidades/${sc.id}`}
              className="ml-auto flex items-center gap-1 text-xs text-muted-foreground hover:text-info transition-colors">
      ...
      {isPendente && bulkActive && (
        <div
          className="border-t border-border px-4 py-2.5 bg-muted/40 flex items-center justify-between cursor-pointer"
          onClick={onToggle}
        >
          ...
          <Link href={`/compras/necessidades/${sc.id}`}
            ...
            className="flex items-center gap-1 text-xs text-muted-foreground hover:text-info transition-colors">
            Ver SC <ChevronRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      )}
        ...
        {showObs && (
          <div className="px-4 pt-3 pb-2 border-b border-gray-700">
            ...
            <Textarea
              ...
              onChange={(e) => setObs(e.target.value)}
              rows={2}
              className="text-sm resize-none bg-gray-800 border-gray-600 text-white placeholder:text-gray-400 focus:border-red-400"
            />
          </div>
        )}
    ...
    <div className="px-8 py-8 max-w-3xl pb-32">
        ...
        <div className="flex items-center gap-2 mb-3">
          <button onClick={toggleAll}
            className={cn("flex items-center gap-2 text-sm transition-colors",
              allSelected ? "text-info" : someSelected ? "text-blue-500" : "text-muted-foreground hover:text-muted-foreground")}>
            {allSelected
      ...
      {!loading && items.length === 0 && !error && (
        <div className="flex flex-col items-center justify-center py-20 text-center text-muted-foreground">
          ...
          {tab === "PENDENTE" && (
            <p className="text-xs mt-1 text-muted-foreground/60">Quando uma SC for encaminhada para você, ela aparecerá aqui.</p>
          )}
        </div>
      )}

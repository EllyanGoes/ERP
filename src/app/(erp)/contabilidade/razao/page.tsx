...
import { useFormatoContabil, FormatoToggle, fmtSaldo, fmtColuna, saldoAnormal, type NaturezaConta, type FormatoModo } from "@/lib/formato-contabil";
...
const CONTAS_KEY = "contabilidade:razao:contaIds";
type Razao = {
...
// ── Card de um Razão (busca os próprios dados) ────────────────────────────────
// Permite vários razões abertos ao mesmo tempo, cada um na sua conta.
function RazaoCard({ contaId, titulo, range, modo, empresaNome, onOpenConta, onRemove }: {
  contaId: string; titulo: string; range: DateRange; modo: FormatoModo;
  empresaNome: string | null; onOpenConta: (id: string) => void; onRemove: () => void;
}) {
  ...
  function baixarPdf() {
    ...
    gerarPdfContabil({
      ...
      empresa: empresaNome,
      subinfo: [
        `Conta: ${razao.conta.codigo} — ${titulo} (${nat === "DEVEDORA" ? "Devedora" : "Credora"})`,
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="px-5 py-3 border-b border-border bg-muted flex items-center justify-between gap-3">
        <span className="font-semibold text-foreground min-w-0 truncate">
          ...
          {titulo}
          {razao && <span className="ml-2 text-xs text-muted-foreground">({razao.conta.natureza === "DEVEDORA" ? "Devedora" : "Credora"})</span>}
        </span>
        <div className="flex items-center gap-3 shrink-0">
          {razao && <span className="text-sm text-muted-foreground hidden sm:inline">Saldo anterior: <b className="text-foreground tabular-nums">{fmtSaldo(razao.saldoInicial, modo, razao.conta.natureza as NaturezaConta)}</b></span>}
          <button type="button" onClick={baixarPdf} disabled={!razao} title="Baixar PDF" className="text-muted-foreground hover:text-foreground disabled:opacity-40"><FileDown className="w-4 h-4" /></button>
          <button type="button" onClick={onRemove} title="Fechar este razão" className="text-muted-foreground hover:text-danger"><X className="w-4 h-4" /></button>
        </div>
      </div>

      {loading || !razao ? (
        <div className="flex items-center justify-center py-12 text-muted-foreground gap-2"><Loader2 className="w-5 h-5 animate-spin" /> Carregando…</div>
      ) : (
...
export default function RazaoPage() {
  ...
  // Click-through (?contaId=&from=&to=) tem prioridade; senão, restaura as
  // contas abertas (persistidas ao trocar de página).
  useEffect(() => {
    const sp = new URLSearchParams(window.location.search);
    const cId = sp.get("contaId");
    const from = sp.get("from");
    const to = sp.get("to");
    if (cId) setContaIds([cId]);
    else { try { const saved = localStorage.getItem(CONTAS_KEY); if (saved) { const arr = JSON.parse(saved); if (Array.isArray(arr)) setContaIds(arr.filter((x) => typeof x === "string")); } } catch { /* ignore */ } }
    if (from || to) setRange((r) => ({ from: from || r.from, to: to || r.to }));
  }, []);
  // Persiste as contas abertas (sobrevive à navegação entre páginas).
  useEffect(() => {
    try { localStorage.setItem(CONTAS_KEY, JSON.stringify(contaIds)); } catch { /* ignore */ }
  }, [contaIds]);
  ...
  const addConta = useCallback((id: string) => {
    if (!id) return;
    setContaIds((prev) => (prev.includes(id) ? prev : [...prev, id]));
  }, []);
  const removeConta = useCallback((id: string) => setContaIds((prev) => prev.filter((x) => x !== id)), []);
      ...
      <div className="px-8 pb-8 space-y-4">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="w-[34rem] max-w-full">
            <ComboboxWithCreate
              value=""
              onChange={(id) => addConta(id)}
              placeholder="Adicionar conta ao razão..."
        ...
        {contaIds.length === 0 ? (
          <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-xl">
            ...
            <p className="text-xs mt-1">Pode abrir vários razões ao mesmo tempo. Contas sintéticas agregam as analíticas.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {contaIds.map((id) => (
              <RazaoCard
                key={id}
                contaId={id}
                titulo={tituloDaConta(id)}
                range={range}
                modo={modo}
                empresaNome={empresaNome}
                onOpenConta={addConta}
                onRemove={() => removeConta(id)}
              />
            ))}
          </div>
        )}

...
export default function ConcorrenteDetailPage() {
  ...
  async function geocodificar() {
    ...
    if (res.ok) {
      setData((d) => (d ? { ...d, latitude: json.data.latitude, longitude: json.data.longitude } : d));
    ...
    <div>
      ...
      <div className="px-8 pb-10 space-y-6">
        <div className="flex items-center gap-2 text-sm">
          ...
          {temGeo ? (
            <span className="text-muted-foreground">Georreferenciado ({data.latitude!.toFixed(5)}, {data.longitude!.toFixed(5)})</span>
          ) : (
            <span className="text-muted-foreground">Sem localização no mapa — preencha o endereço e clique em “Localizar no mapa”.</span>
          )}
        </div>

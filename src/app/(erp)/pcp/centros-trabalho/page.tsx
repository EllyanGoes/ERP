...
const inputCls =
  "w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500";
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-8 pb-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Cadastre os recursos da produção (forno, secador, prensa…).</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-border">
                {centros.map((c) => (
                  <tr key={c.id} className="hover:bg-muted/60">
                    <td className="px-4 py-2.5 font-mono text-foreground">{c.codigo}</td>
                    <td className="px-4 py-2.5 text-foreground">{c.nome}</td>
                    <td className="px-4 py-2.5 text-muted-foreground">{c.tipo ? TIPO_LABEL[c.tipo] : "—"}</td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-muted-foreground">
                      {c.capacidadePadrao != null ? `${c.capacidadePadrao} ${c.unidadeCapacidade ?? ""}`.trim() : "—"}
                    </td>
                    <td className="px-4 py-2.5 text-center text-muted-foreground">{c.codApl ?? "—"}</td>
                    <td className="px-4 py-2.5 text-center">
                      <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium", c.ativo ? "bg-success/10 text-success" : "bg-muted text-muted-foreground")}>
                        {c.ativo ? "Sim" : "Não"}
                      </span>
                    </td>
                    <td className="px-4 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        <button type="button" onClick={() => abrirEdicao(c)} title="Editar" className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground">
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button type="button" onClick={() => excluir(c)} title="Excluir" className="p-1.5 rounded-lg text-muted-foreground hover:bg-danger/10 hover:text-danger">

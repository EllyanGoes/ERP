...
const inputCls = "rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-8 pb-8 space-y-4">
        ...
        {novo && (
          <div className="rounded-xl border border-cyan-200 dark:border-cyan-500/30 bg-cyan-50/40 p-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
              <div className="md:col-span-2">
                ...
                {novo.item ? (
                  <div className="flex items-center justify-between rounded-lg border border-border bg-card px-3 py-2 text-sm">
                    <span className="truncate"><span className="font-mono text-muted-foreground text-xs mr-2">{novo.item.codigo}</span>{novo.item.descricao}</span>
              ...
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Período (mês) *</label>
                <input type="month" className={inputCls + " w-full"} value={novo.periodo} onChange={(e) => setNovo({ ...novo, periodo: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Quantidade (milheiros) *</label>
            ...
            <div className="mt-3 flex items-center justify-end gap-2">
              <button onClick={() => setNovo(null)} className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-sm text-muted-foreground hover:bg-muted"><X className="w-4 h-4" /> Cancelar</button>
        ...
        {/* MPS */}
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5"><CalendarRange className="w-4 h-4 text-cyan-500" /> Plano Mestre (demanda)</h3>
          {loading ? (
            <div className="flex items-center justify-center py-10 text-muted-foreground gap-2 text-sm"><RefreshCw className="w-4 h-4 animate-spin" /> Carregando…</div>
          ) : linhas.length === 0 ? (
            <p className="text-xs text-muted-foreground py-6 text-center rounded-xl border border-dashed border-border">Sem demanda planejada. Use o botão Nova demanda (manual) por produto e mês.</p>
          ) : (
                ...
                <tbody className="divide-y divide-border">
                  {linhas.map((l) => (
                    <tr key={l.id} className="hover:bg-muted/60">
                      <td className="px-4 py-2 text-foreground"><span className="font-mono text-muted-foreground text-xs mr-2">{l.item.codigo}</span>{l.item.descricao}</td>
                      <td className="px-4 py-2 text-muted-foreground tabular-nums">{l.periodo}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-foreground">{Number(l.quantidade)}</td>
                      <td className="px-4 py-2 text-center text-xs text-muted-foreground">{ORIGEM_LABEL[l.origem] ?? l.origem}</td>
                      <td className="px-4 py-2 text-right"><button onClick={() => excluir(l)} className="p-1 text-muted-foreground/60 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {mrp && (
          <div>
            <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5"><Calculator className="w-4 h-4 text-cyan-500" /> Necessidades (MRP) — {mrp.produtosPlanejados} produto(s)</h3>
            {mrp.semEngenharia.length > 0 && (
              <div className="mb-2 rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning flex items-start gap-2">
            ...
            {mrp.necessidades.length === 0 ? (
              <p className="text-xs text-muted-foreground py-6 text-center rounded-xl border border-dashed border-border">Nenhuma necessidade (sem BOM ou sem demanda).</p>
            ) : (
              <div className="rounded-xl border border-border bg-card overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-muted text-xs text-muted-foreground uppercase tracking-wider">
                  ...
                  <tbody className="divide-y divide-border">
                    {mrp.necessidades.map((n) => (
                      <tr key={n.insumoItemId} className={cn(n.liquida > 0 && "bg-danger/10")}>
                        <td className="px-4 py-2 text-foreground"><span className="font-mono text-muted-foreground text-xs mr-2">{n.codigo}</span>{n.descricao}</td>
                        <td className="px-4 py-2 text-muted-foreground text-xs">{CAT_LABEL[n.categoria] ?? n.categoria}</td>
                        <td className="px-4 py-2 text-right tabular-nums text-foreground">{n.bruta}</td>
                        <td className="px-4 py-2 text-right tabular-nums text-muted-foreground">{n.disponivel}</td>
                        <td className={cn("px-4 py-2 text-right tabular-nums font-medium", n.liquida > 0 ? "text-danger" : "text-success")}>{n.liquida}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <p className="text-[11px] text-muted-foreground mt-1.5">Bruta = demanda × estrutura (BOM). A comprar = bruta − estoque. (Gross-up de perda por etapa entra numa evolução.)</p>
          </div>
        )}
      </div>
    </div>

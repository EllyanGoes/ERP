  ...
  try {
    ...
    const minuta = await prisma.minuta.findUnique({
      ...
      include: {
        itens: true,
        localEstoque: { select: { id: true, nome: true } },
        pedidoVenda: { select: { estoqueOrigemEmpresaId: true, pedidoVendaOrigemId: true } },
      },
    });
    if (!minuta) return NextResponse.json({ error: "Minuta não encontrada" }, { status: 404 });

    // Minuta de um PEDIDO DE ENTREGA de venda à ordem (Tramontin): faz a baixa
    // normal (real) e, ao ENTREGUE, dispara a compra virtual + financeiro na
    ...
    if (Array.isArray(body.itens)) {
      ...
      if (minuta.status === "ENTREGUE" || effectiveStatus === "ENTREGUE") {
        ...
        // Rede de segurança do faturamento (o título nasce na CONFIRMAÇÃO):
        // garante o CR de pedidos confirmados antes da regra e contabiliza.
        await faturarPedido(minuta.pedidoVendaId).catch((e) =>
          console.error(`[Minutas] faturarPedido(${minuta.pedidoVendaId}) falhou:`, e));

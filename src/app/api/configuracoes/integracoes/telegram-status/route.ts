...
export async function POST(req: NextRequest) {
  try {
    ...
    const token  = records.find((r) => r.chave === "tg_bot_token")?.valor;
    const chatId = body.chatId ?? records.find((r) => r.chave === "tg_chat_id")?.valor;
    ...
    const getMeData = await getMeRes.json() as { ok: boolean; result?: { username?: string } };
    ...
    if (!msgRes.ok) {
      const err = await msgRes.json().catch(() => ({})) as { description?: string };
      return NextResponse.json({ connected: false, reason: err.description ?? "Não foi possível enviar mensagem para o chat" });
    }

    return NextResponse.json({ connected: true, reason: `Bot: @${getMeData.result?.username ?? "Bot"}` });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    return NextResponse.json({ connected: false, reason: msg });
  }

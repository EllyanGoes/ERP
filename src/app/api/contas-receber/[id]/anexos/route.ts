...
// Anexos do título a RECEBER (fatura, boleto, comprovante…) — arquivos no Vercel
...
export async function GET(_: NextRequest, { params }: Params) {
  ...
  const anexos = await prisma.anexoTitulo.findMany({
    where: { contaReceberId: params.id },
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const conta = await prisma.contaReceber.findUnique({ where: { id: params.id }, select: { id: true } });
  ...
  const blob = await put(`titulos/receber/${params.id}/${Date.now()}-${file.name}`, file, { access: "public" });

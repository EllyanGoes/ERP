...
export default function ContasPagarTable({ contas }: { contas: ContaRow[] }) {
  ...
  async function handlePagamento() {
    ...
    setSaving(true);
    await fetch(`/api/contas-pagar/${selected.id}`, {
      method: "PATCH",
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            {selected && <p className="text-sm text-gray-500">{selected.numero} — {selected.descricao}</p>}
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div><Label>Valor Pago (R$)</Label><Input type="number" step="0.01" min="0" value={valorPago} onChange={(e) => setValorPago(e.target.value)} className="mt-1" /></div>
            <div><Label>Data do Pagamento</Label><Input type="date" value={dataPag} onChange={(e) => setDataPag(e.target.value)} className="mt-1" /></div>
            <div>
              <Label>Forma de Pagamento</Label>
              <Select value={forma} onValueChange={setForma}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Selecionar..." /></SelectTrigger>
                <SelectContent>
                  {["PIX", "Boleto", "Transferência", "Cartão de Crédito", "Dinheiro"].map((f) => (

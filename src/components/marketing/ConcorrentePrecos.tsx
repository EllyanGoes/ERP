  ...
  useEffect(() => {
    fetch("/api/itens?limit=500")
      ...
      .then((j) => {
        ...
        setItens(
          lista.map((it) => ({ value: it.id, label: it.descricao, code: it.codigo })),
        );
  ...
  async function adicionar() {
    ...
    try {
      const res = await fetch(`/api/marketing/concorrentes/${concorrenteId}/precos`, {
        ...
        body: JSON.stringify({ itemId: itemId || null, produtoNome: nome, preco: precoNum, unidade: unidade || null }),
      });
      if (res.ok) {
        const json = await res.json();
        setPrecos((p) => [...p, json.data]);
        setItemId(""); setProdutoNome(""); setPreco(""); setUnidade("");
      } else {
  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-border bg-muted">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Compare o preço do concorrente com o nosso preço de venda.</p>
      </div>
      ...
      <div className="p-4 border-b border-border bg-muted/30">
        <div className="grid grid-cols-12 gap-2 items-end">
          ...
          <div className="col-span-1">
            <Button type="button" onClick={adicionar} disabled={salvando} className="w-full h-10">
              {salvando ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
            </Button>
          </div>
        </div>
        {nossoPrecoNovo != null && (
          <p className="text-[11px] text-muted-foreground mt-1.5">Nosso preço deste item: <strong>{formatBRL(nossoPrecoNovo)}</strong></p>
        )}

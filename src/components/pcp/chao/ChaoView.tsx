  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-gray-100 flex-wrap">
        <div>
          <h1 className="text-lg font-semibold text-gray-900">Chão de Fábrica</h1>
          <p className="text-xs text-gray-400">{chao ? `${chao.fluxo.nome} · v${chao.fluxo.versao}` : "Fluxo de processo compartilhado"}</p>
        </div>
        <div className="flex items-center gap-1.5 ml-2">
          <CalendarDays className="w-4 h-4 text-gray-400" />
          <input type="date" value={dia} onChange={(e) => setDia(e.target.value)} className="h-9 rounded-lg border border-gray-300 px-2 text-sm" />
        </div>
        {chao && (
          <span className="text-xs text-gray-500">
            Planejado do dia: <b>{chao.plano.length}</b> produto(s)
            {chao.plano.some((p) => p.origem === "MPS") && <span className="text-amber-600"> (derivado do MPS)</span>}
          </span>
        )}
        <div className="flex-1" />
        {aviso && <span className="text-xs text-emerald-700">{aviso}</span>}
        <button onClick={load} className="p-2 rounded-lg text-gray-400 hover:bg-gray-100" title="Recarregar"><RefreshCw className="w-4 h-4" /></button>
        <button
      ...
      <div className="flex-1 relative">
        {loading ? (
          <div className="absolute inset-0 flex items-center justify-center text-gray-400"><Loader2 className="w-6 h-6 animate-spin" /></div>
        ) : !chao ? (
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-80 bg-white border-l border-gray-200 shadow-xl z-20 flex flex-col">
      <div className="flex items-center justify-between px-4 h-14 border-b border-gray-100 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-wide text-gray-400 leading-none">{s.label}</p>
            <p className="text-sm font-semibold text-gray-800 truncate">{node.data.label || "Sem nome"}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {isEstoque && (
          <>
            ...
            {!node.data.localEstoqueId ? (
              <p className="text-xs text-amber-600">Nó sem local de estoque configurado (defina no editor do fluxo).</p>
            ) : !saldo || saldo.itens.length === 0 ? (
              <p className="text-xs text-gray-400">Sem produtos nesta fase.</p>
            ) : (
              <div className="border border-gray-100 rounded-lg divide-y divide-gray-50">
                {saldo.itens.map((i) => (
                  <div key={i.itemId} className="flex items-center justify-between px-3 py-2 text-sm">
                    <span className="min-w-0">
                      {i.codigo && <span className="font-mono text-xs text-gray-400 mr-1">{i.codigo}</span>}
                      <span className="text-gray-700">{i.descricao}</span>
                    </span>
                    <span className="font-medium text-gray-900 shrink-0">{fmtQty(i.quantidade)} <span className="text-xs text-gray-400">{i.unidade ?? ""}</span></span>
                  </div>
                ))}
              </div>
            )}
        ...
        {isOperacao && (
          <>
            <p className="text-sm font-medium text-gray-700">Gerar Ordem de Produção</p>
            <p className="text-xs text-gray-400">Escolha o produto e a quantidade. A OP usa o fluxo do produto (engenharia).</p>
            <div>
              <label className="text-[11px] font-medium text-gray-500 mb-1 block">Produto</label>
              {opItem ? (
                <div className="flex items-center justify-between rounded-lg border border-gray-200 px-2.5 py-1.5 text-sm">
                  <span className="truncate text-gray-700">{opItem.descricao}</span>
                  <button onClick={() => setOpItem(null)}><X className="w-3.5 h-3.5 text-gray-300 hover:text-gray-500" /></button>
                </div>
              ) : (
            ...
            <div>
              <label className="text-[11px] font-medium text-gray-500 mb-1 block">Quantidade planejada</label>
              <input value={opQtd} onChange={(e) => setOpQtd(e.target.value)} inputMode="decimal" placeholder="ex.: 10"
                className="w-full h-9 rounded-lg border border-gray-300 px-2.5 text-sm" />
            </div>
            <button
              onClick={onGerarOP}
        ...
        {!isEstoque && !isOperacao && (
          <p className="text-xs text-gray-400">Etapa de {s.label.toLowerCase()} — sem saldo nem geração de OP.</p>
        )}
      </div>
    </div>
  );

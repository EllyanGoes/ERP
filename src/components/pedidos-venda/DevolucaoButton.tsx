          ...
          <div className="fixed inset-0 z-[9999] flex justify-end bg-black/40" onClick={() => !salvando && setOpen(false)}>
            <div className="h-full w-full max-w-xl bg-white shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
                ...
                {loading ? (
                  ...
                  <>
                    <div className="space-y-1.5">
                      <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Resolução</p>
                      <div className="grid grid-cols-3 gap-2">
                        {([
                          ...
                          <button
                            ...
                            className={`rounded-lg border px-2 py-2 text-left transition-colors ${tipo === val ? "border-orange-400 bg-orange-50 ring-1 ring-orange-200" : "border-gray-200 hover:border-gray-300"}`}
                          >
              ...
              <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-100">
                <span className="text-sm text-gray-500">{tipo === "ESTORNO" ? "Total a estornar" : "Total em crédito"} <span className="font-bold text-gray-900 tabular-nums">{formatBRL(total)}</span></span>

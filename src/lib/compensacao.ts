...
export type TituloNetting = { id: string; saldo: number; netted: number };
export type Netting = {
  min: number;
  sumR: number;
  sumP: number;
  ...
  residual: number;
  nettedR: TituloNetting[];
  nettedP: TituloNetting[];
};

/**
 * Calcula o "netting" (compensação líquida) dos títulos selecionados — SEM exigir
 * mesmo parceiro nos dois lados. O total compensado é `min(ΣReceber, ΣPagar)`;
 * cada título recebe uma parcela compensada (`netted`) preenchendo na ordem dada
 * (o lado menor fica 100% compensado). Quem decide como APLICAR (parcial x nova
 * parcela) é o chamador. Retorna null se não há o que compensar (min ≤ 0).
 */
export function calcularNetting(receber: TituloSaldo[], pagar: TituloSaldo[]): Netting | null {
  ...
  // Preenche cada lado até `min` (o lado menor, cujo Σ = min, fica todo compensado).
  const preencher = (list: TituloSaldo[]): TituloNetting[] => {
    let rem = min;
    return list.map((t) => {
      const n = r2(Math.min(t.saldo, Math.max(0, rem)));
      rem = r2(rem - n);
      return { id: t.id, saldo: r2(t.saldo), netted: n };
    });
  };
  return {
    min: r2(min),
    sumR,
    sumP,
    maiorLado: sumR >= sumP ? "RECEBER" : "PAGAR",
    residual: r2(Math.abs(sumR - sumP)),
    nettedR: preencher(receber),
    nettedP: preencher(pagar),
  };
}
...
export type TituloAberto = {
  id: string; numero: string; descricao: string; parceiro: string;
  dataVencimento: Date | null; saldo: number;
};

/**
 * TODOS os títulos em aberto (a receber e a pagar) da empresa, para a seleção
 * LIVRE da compensação — o encontro NÃO exige mesmo parceiro nos dois lados.
 */
export async function titulosAbertosDaEmpresa(empresaId: string): Promise<{ receber: TituloAberto[]; pagar: TituloAberto[] }> {
  const [crs, cps] = await Promise.all([
    prismaSemEscopo.contaReceber.findMany({
      where: { empresaId, intragrupo: false, status: { in: [...STATUS_ABERTOS] }, clienteId: { not: null } },
      select: { id: true, numero: true, descricao: true, valorOriginal: true, valorPago: true, dataVencimento: true, cliente: { select: { razaoSocial: true } } },
      orderBy: { dataVencimento: "asc" },
    }),
    prismaSemEscopo.contaPagar.findMany({
      where: { empresaId, intragrupo: false, status: { in: [...STATUS_ABERTOS] }, fornecedorId: { not: null } },
      select: { id: true, numero: true, descricao: true, valorOriginal: true, valorPago: true, dataVencimento: true, fornecedor: { select: { razaoSocial: true } } },
      orderBy: { dataVencimento: "asc" },
    }),
  ]);
  const receber = crs
    .filter((c) => saldoTitulo(c) > 0.005)
    .map((c) => ({ id: c.id, numero: c.numero, descricao: c.descricao, parceiro: c.cliente?.razaoSocial ?? "—", dataVencimento: c.dataVencimento, saldo: saldoTitulo(c) }));
  const pagar = cps
    .filter((c) => saldoTitulo(c) > 0.005)
    .map((c) => ({ id: c.id, numero: c.numero, descricao: c.descricao, parceiro: c.fornecedor?.razaoSocial ?? "—", dataVencimento: c.dataVencimento, saldo: saldoTitulo(c) }));
  return { receber, pagar };
}

 ...
 * pelo servidor (pooler não aceita roles avulsas; sem IPv6 p/ conexão direta),
 * então o import roda NO app, em lotes limitados por `budgetMs` (timeout da
 * Vercel): a execução para no meio e retorna `restantes > 0`; re-chamar continua
 * de onde parou. O lançamento de abertura só sai quando restantes = 0.
...
import { contabilizarTituloPagar, registrarLancamento } from "@/lib/contabilidade";
import {
  garantirContasFolha,
...
const AVULSOS: Avulso[] = [
  ...
  { chave: "a07-08", empresa: "T", venc: "2026-07-10", desc: "RECIBO DE FÉRIAS - ALTAMIDISON (aquisitivo 17/05/2024 a 16/05/2025)", valor: 4787.48, natureza: "3.01", colaborador: "ALTAMIDIS" },
  ...
  { chave: "a08-03", empresa: "T", venc: "2026-08-10", desc: "VOCE TELECOM - FÁBRICA - MÊS 07", valor: 334.9, natureza: "4.02", fornecedor: "VOCE TELECOMUNICACOES LTDA" },
  ...
  { chave: "a08-07", empresa: "T", venc: "2026-08-15", desc: "VOCE TELECOM - PORTO - MÊS 07", valor: 142.41, natureza: "4.02", fornecedor: "VOCE TELECOMUNICACOES LTDA" },
  ...
  { chave: "a08-12", empresa: "T", venc: "2026-08-17", desc: "SANTA APOLONIA - MOACIR BERNADINI - PARC 02", valor: 7100.0, natureza: "2.01", fornecedor: "MOACIR BERNARDINI", doc: "964-02", parcela: { n: 2, total: null, grupo: "santa-apolonia" } },
  { chave: "a08-13", empresa: "T", venc: "2026-09-16", desc: "SANTA APOLONIA - MOACIR BERNADINI - PARC 03", valor: 7100.0, natureza: "2.01", fornecedor: "MOACIR BERNARDINI", doc: "964-03", parcela: { n: 3, total: null, grupo: "santa-apolonia" } },
  ...
  // ── Aba 08 — Atlas
  { chave: "a08-17", empresa: "A", venc: "2026-08-21", desc: "MACHADO DISTRIBUIÇÃO COMÉRCIO DE PEÇAS - PARC 2/5", valor: 2087.08, natureza: "2.04", fornecedor: "MACHADO DISTRIBUIÇÃO COMERCIO DE PEÇAS", doc: "50760", parcela: { n: 2, total: 5, grupo: "machado-5x" } },
  { chave: "a08-18", empresa: "A", venc: "2026-09-11", desc: "MACHADO DISTRIBUIÇÃO COMÉRCIO DE PEÇAS - PARC 3/5", valor: 2087.08, natureza: "2.04", fornecedor: "MACHADO DISTRIBUIÇÃO COMERCIO DE PEÇAS", doc: "50761", parcela: { n: 3, total: 5, grupo: "machado-5x" } },
  { chave: "a08-19", empresa: "A", venc: "2026-10-01", desc: "MACHADO DISTRIBUIÇÃO COMÉRCIO DE PEÇAS - PARC 4/5", valor: 2087.08, natureza: "2.04", fornecedor: "MACHADO DISTRIBUIÇÃO COMERCIO DE PEÇAS", doc: "50762", parcela: { n: 4, total: 5, grupo: "machado-5x" } },
  { chave: "a08-20", empresa: "A", venc: "2026-10-21", desc: "MACHADO DISTRIBUIÇÃO COMÉRCIO DE PEÇAS - PARC 5/5", valor: 2087.06, natureza: "2.04", fornecedor: "MACHADO DISTRIBUIÇÃO COMERCIO DE PEÇAS", doc: "50763", parcela: { n: 5, total: 5, grupo: "machado-5x" } },
...
export type ResultadoImportCp = {
  ...
  porPassivo: Record<string, number>;
  fornecedoresCriados: string[];
  colaboradoresSemMatch: string[];
  log: string[];
};

export async function executarImportPlanilhaCp(
  opts: { dry?: boolean; budgetMs?: number } = {},
): Promise<ResultadoImportCp> {
  ...
  // Fornecedores: get-or-create por nome. Reusa APENAS com igualdade exata
  // normalizada (razão ou fantasia) — match parcial em prod vinculava errado
  // (ex.: "CEA" batia em "CEARA ELETRO"). Sem match exato, cria novo.
  ...
  async function fornecedorId(nome: string): Promise<string | null> {
    ...
    const hit = candidatos.find((c) =>
      ...
      (c.nomeFantasia ?? "").trim().toUpperCase().replace(/\s+/g, " ") === norm) ?? null;
  ...
  for (const l of linhas) {
    ...
    // Budget (rota admin): estourou o tempo → o resto fica p/ a próxima chamada.
    if (!dry && Date.now() - t0 > budgetMs) { restantes++; continue; }
    ...
    if (colisao) {
      log.push(`~ pulado (já existe ${colisao.numero}): ${l.desc} ${l.venc}`);
      stats.pulados++;
      continue;
    }
    ...
    await contabilizarTituloPagar(cp.id).catch((e) => log.push(`! contabilização falhou ${cp.numero}: ${e instanceof Error ? e.message : String(e)}`));
  ...
  return {
    ...
    restantes,
    totais: stats.total,
    porPassivo: Object.fromEntries(stats.porPassivo),
    fornecedoresCriados: fornCriados,
    colaboradoresSemMatch: Array.from(colabSemMatch),
    log,
  };
}
